...
## Agrivive form validation

This project convention applies when building or reviewing Agrivive mobile forms.

- Each field owns its validation errors. Render the message directly below that field, inside the shared input component, and indicate its invalid state with a border plus readable text.
- Keep errors keyed by field, such as `errors.email`, `errors.password`, `errors.confirmPassword`, and `errors.otp`. Show all relevant validation errors from one submission.
- Map structured API validation paths and known error codes to the relevant field. Duplicate-email errors belong to email; password-rule failures to password; mismatches to confirm password; invalid or expired codes to the code field. Preserve a generic invalid-credentials message without claiming which credential is wrong.
- Do not put field validation in a banner above the form or rely on an alert dialog. Genuine connection failures, service failures, and errors that cannot be attributed to a field use one form-level message near the submit button.
- When a field changes, clear or revalidate only its error and dependent validations, such as confirmation after changing the password. Preserve unrelated field errors and entered values.
- Give map selections and uploads their own inline error area. Do not attribute an upload failure to a text field.
- Keep error text at least 14 px with wrapping and accessible announcements. On submission, focus or scroll to the first invalid field without hiding its message behind the keyboard.
- In manual verification, submit several invalid values together, correct one field, and confirm the other errors remain. Check API validation, OTP expiry, and offline submission separately.
